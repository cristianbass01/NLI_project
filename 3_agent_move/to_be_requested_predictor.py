...
class ToBeRequestedPredictor():
    def __init__(self, model_path, model_name, cuda = False):
        ...
        self.mlb = pickle.load(open(os.path.join(model_path, 'MOVE_AGENT_REQ_mlb.pkl'), 'rb'))
        ...
        self.intent_classifier.load_state_dict(load(os.path.join(model_path, 'MOVE_AGENT_REQ_' + model_name + '.pt')))
        if cuda:
    ...
    def predict(self, utterance):
        with torch.no_grad():
            ...
            to_be_req = self.mlb.inverse_transform(out)[0]
            if "NOTHING" in to_be_req:
                to_be_req = set()
            return list(to_be_req)
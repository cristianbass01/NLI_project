...
# Models are available on https://www.kaggle.com/datasets/cristianbassotto/saved-models/
...
class Agent:
    ...
    def predict_slot_filling(self):
        ...
        for index in range(len(sentence)):
            if prediction[index] != 'O':
                ...
                if self.index > 1 and slot_name in self.history[self.index-1]['all_slots'] and slot_value == 'same':
                    slot_value = self.history[self.index-1]['all_slots'][slot_name]
                    
                self.history[self.index]['slot_filling'][slot_name] = map_slot_value(slot_value)
        
        if 'all_slots' not in self.history[self.index]:
            self.history[self.index]['all_slots'] = {}

        self.history[self.index]['all_slots'].update(self.history[self.index]['slot_filling'])
    ...
    def get_agent_response(self):
        to_be_requested = self.history[self.index]['to_be_requested']
        to_be_provided = self.history[self.index]['to_be_provided']
        user_dialogue_act = self.history[self.index]['dialogue_act']
        response = self.database.retrieve_agent_response(user_dialogue_act, to_be_requested, to_be_provided)
    ...
    def get_to_be_retrieved_historical_utterance(self):
        ...
        historical_slots = sorted(set(self.history[self.index]['all_slots'].keys()))
        ...
        historical_utterance += " | " + " , ".join(historical_slots)
...
sentence = 'any sort of food would be fine , as long as it be a bit expensive . could I get the phone number for your recommendation ?'
print('Adding user utterance:', sentence)
agent.add_user_utterance(sentence)
print('Historical utterance:', agent.get_dialogue_acts_historical_utterance())
print('Predicting dialogue act...')

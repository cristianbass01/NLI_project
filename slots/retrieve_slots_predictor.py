...
class RetrieveSlotsPredictorLSTM:
    def __init__(self, model_path, model_name, cuda = False):
        ...
        self.mlb = pickle.load(open(os.path.join(model_path, 'retrieve_mlb.pkl'), 'rb'))
        ...
        self.intent_classifier.load_state_dict(load(os.path.join(model_path, 'RETR_' + model_name + '.pt')))
        if cuda:
